...
Tests how well facts are preserved after caveman compression by using an LLM
...
Usage:
    ...
    python benchmark/factual_preservation/run_factual_benchmark.py --output results.json
...
# Add parent directory to path to import caveman_compress
...
def compress_text(text, model="gpt-4o"):
    """Compress text using caveman compression - returns only the compressed text"""
    import subprocess

    try:
        result = subprocess.run(
            ["python3", "caveman_compress.py", "compress", "--model", model, text],
            capture_output=True,
            text=True,
            timeout=60
        )
...
def run_benchmark(test_data_path, output_path=None, model="gpt-4o", verbose=True):
    """
    Run the factual preservation benchmark
    ...
    if verbose:
        print(f"Running Factual Preservation Benchmark")
        print(f"Model: {model}")
    ...
    if output_path:
        ...
        output_file = Path(__file__).parent / "reports" / f"factual_preservation_{timestamp}.json"
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark factual preservation in caveman compression"
    )
    parser.add_argument(
        '--test-data',
    ...
    parser.add_argument(
        '--output',
        help='Output file path for results (default: results_TIMESTAMP.json)'
    )
    parser.add_argument(
        '--model',
        default='gpt-4o',
